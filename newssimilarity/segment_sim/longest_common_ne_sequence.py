...
class LongestCommonNESequence(SegmentSimMeasurement):
    ...
    def calculate_similarity(self):
        ...
        table = [[0] * (len(s1_nes_preprocessed) + 1) for i in range(0, len(s2_nes) + 1)]
        for i in range(1, len(table)):
            for k in range(1, len(table[0])):
                if s2_nes_preprocessed[i - 1] == s1_nes_preprocessed[k - 1]:
