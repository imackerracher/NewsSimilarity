"""
Difference: return pos tag as well as token
"""
...
class Preprocessor:
    ...
    def process(self):
        ...
        if self.stopword_removal:
            ...
            processed_tokens = [(self.case(token[0]), token[1]) for token in processed_tokens if token[0] not in stop]

        if self.stemming:
            if self.stemmer == 'lancaster':
                ls = LancasterStemmer()
                processed_tokens = [(self.case(ls.stem(token[0])), token[1]) for token in processed_tokens]
            elif self.stemmer == 'snowball':
                ss = SnowballStemmer('english')
                processed_tokens = [(self.case(ss.stem(token[0])), token[1]) for token in processed_tokens]

            else:
                ps = PorterStemmer()
                processed_tokens = [(self.case(ps.stem(token[0])), token[1]) for token in processed_tokens]

        if self.lemmatization:
            lemma = nltk.wordnet.WordNetLemmatizer()
            processed_tokens = [(self.case(lemma.lemmatize(token[0], token[1])), token[1]) for token in processed_tokens]
        ...
        if not self.stopword_removal and not self.stemming and not self.lemmatization:
            processed_tokens = [(self.case(token[0]), token[1]) for token in processed_tokens]
